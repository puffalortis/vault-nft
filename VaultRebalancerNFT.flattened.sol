...
pragma solidity >=0.4.16 >=0.5.0 >=0.6.2 >=0.8.4 ^0.8.20 ^0.8.24 ^0.8.30;
...
// src/VaultRebalancerNFT.sol

contract VaultRebalancerNFT is ERC721 {
    /// -----------------------------------------------------------------------
    /// Token addresses (Base mainnet, checksummed)
    /// -----------------------------------------------------------------------

    IERC20 public immutable cbBTC;
    IERC20 public immutable DOWN;
    ...
    constructor(address _cbBTC, address _down)
        ERC721("Vault Rebalancer NFT", "VRNFT")
    {
        cbBTC = IERC20(_cbBTC);
        DOWN  = IERC20(_down);
    }
    ...
    function deposit(
        ...
        if (cbBTCAmount > 0) {
            ...
            positions[tokenId].cbBTCBalance += cbBTCAmount;
        }

        if (downAmount > 0) {
            DOWN.transferFrom(msg.sender, address(this), downAmount);
            positions[tokenId].downBalance += downAmount;
        }
    }
    ...
    function withdraw(
        ...
        VaultPosition storage p = positions[tokenId];

        require(p.cbBTCBalance >= cbBTCAmount, "Insufficient cbBTC");
        require(p.downBalance  >= downAmount,  "Insufficient DOWN");

        if (cbBTCAmount > 0) {
            p.cbBTCBalance -= cbBTCAmount;
            cbBTC.transfer(msg.sender, cbBTCAmount);
        }

        if (downAmount > 0) {
            p.downBalance -= downAmount;
            DOWN.transfer(msg.sender, downAmount);
        }
    }
    ...
    function balances(uint256 tokenId)
        external
        view
        returns (uint256 cbBTCBal, uint256 downBal)
    {
        VaultPosition memory p = positions[tokenId];
        return (p.cbBTCBalance, p.downBalance);
    }
}
